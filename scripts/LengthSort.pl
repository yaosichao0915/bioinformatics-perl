...
my $first_id;
my $second_id;
if( $first_line =~ /(^\S*:\d*:\d*:\d*:\d*)/ ){
	$first_id = $1;
}else{
	die "error: header lines in $files[0] do not appear to be in standard Illumina format\n";
}
if( $paired ){
	if( $second_line =~ /(^\S*:\d*:\d*:\d*:\d*)/ ){
		$second_id = $1;
	}else{
		die "error: header lines in $files[1] do not appear to be in standard Illumina format\n";
	}
...
until( eof(FIRST) ){
	...
	if( $paired ){
		my $first_header_id;
		my $second_header_id;
		if( $first_header_line1 =~ /(^\S*:\d*:\d*:\d*:\d*)/ ){
			$first_header_id = $1;
		}else{
			die "error: header line in $files[0] does not appear to be in standard Illumina format\n";
		}
		if( $second_header_line1 =~ /(^\S*:\d*:\d*:\d*:\d*)/ ){
			$second_header_id = $1;
		}else{
			die "error: header line in $files[1] does not appear to be in standard Illumina format\n";
		}
